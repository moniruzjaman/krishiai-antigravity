...
const App: React.FC = () => {
  ...
  const [handledPermissions, setHandledPermissions] = useState(() => {
    return localStorage.getItem('agritech_permissions_granted') === 'true' ||
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case View.CHAT:
        return <ChatBot user={user} userRank={user.progress.rank} userCrops={user.myCrops} onBack={() => handleNavigate(View.HOME)} onAction={() => handleAction(20)} userSettings={user.settings} />;
      case View.SEARCH:
        return <SearchTool onBack={() => handleNavigate(View.HOME)} onAction={() => handleAction(10)} onSaveReport={handleSaveReport} userSettings={user.settings} />;
      case View.ANALYZER:
        return <Analyzer onBack={() => handleNavigate(View.HOME)} onAction={() => handleAction(50)} onSaveReport={handleSaveReport} userRank={user.progress.rank} userCrops={user.myCrops} onNavigate={handleNavigate} lang={lang} userSettings={user.settings} />;
      case View.WEATHER:
      ...
      case View.LEARNING_CENTER:
        return <LearningCenter onBack={() => handleNavigate(View.HOME)} onAction={handleAction} lang={lang} />;
      case View.PROFILE:
      ...
      case View.CABI_TRAINING:
        return <CABIDiagnosisTraining onBack={() => handleNavigate(View.HOME)} onAction={() => handleAction(100)} onSaveReport={handleSaveReport} lang={lang} user={user} />;
      case View.MAPS:
  ...
  if (!handledPermissions) {
    return <PermissionHub lang={lang} onComplete={() => setHandledPermissions(true)} />;
  }

  return (
    <SpeechContext.Provider value={{ playSpeech, stopSpeech, isSpeaking, speechEnabled, setSpeechEnabled }}>
      <div className={`min-h-screen transition-colors duration-500 ${user.settings?.theme === 'dark' ? 'dark bg-slate-950 text-white' : 'bg-slate-50 text-slate-900'}`}>
