...
import { performSoilHealthAudit, generateSpeech, requestSoilPrecisionParameters, performDeepSoilAudit, groundedAgriSearch } from '../services/geminiService';
...
const SoilExpert: React.FC<SoilExpertProps> = ({ onAction, onBack, onSaveReport, onShowFeedback, lang }) => {
   ...
   const handleAuditSubmit = async (precision: boolean = false) => {
      ...
      try {
         ...
         try {
            const fallback = await groundedAgriSearch(aezData?.name || "local soil", `Soil health audit for pH:${auditInputs.ph} N:${auditInputs.n}`, lang);
            setAdvice(`(SAFE SEARCH) ${fallback.fullText}`);
            if (speechEnabled) playSpeech(`Primary audit failed. Fetching official SRDI results. ${fallback.diagnosis}`);
         } catch (f) {
   ...
   const handlePrecisionSubmit = async (dynamicData: Record<string, string>) => {
      ...
      try {
         ...
         try {
            const fallback = await groundedAgriSearch(aezData?.name || "local soil", `Deep soil audit with ${JSON.stringify(dynamicData)}`, lang);
            setAdvice(`(SAFE SEARCH Deep Audit) ${fallback.fullText}`);
         } catch (f) { alert("Deep Soil Audit Failed."); }
