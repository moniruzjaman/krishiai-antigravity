...
import { getAIYieldPrediction, generateSpeech, decodeBase64, decodeAudioData, groundedAgriSearch } from '../services/geminiService';
...
const AIYieldPredictor: React.FC<AIYieldPredictorProps> = ({ user, onAction, onSaveReport, onShowFeedback, onBack, lang }) => {
  ...
  const handlePredict = async () => {
    ...
    try {
      ...
      try {
        const fallback = await groundedAgriSearch(crop, `Yield potential for ${crop} in ${aez} with ${practice}`, lang);
        setPrediction(`(SAFE SEARCH Strategic Report) ${fallback.fullText}`);
        if (fallback.fullText) playTTS(`Primary forecast failed. Pulling official research models. ${fallback.diagnosis}`);
      } catch (f) {
