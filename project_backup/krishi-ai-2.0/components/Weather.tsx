...
import { getLiveWeather, generateSpeech, decodeBase64, decodeAudioData, generateGroundedWeatherReport } from '../services/geminiService';
import { WeatherData, ForecastDay, GroundingChunk, Language } from '../types';
...
const Weather: React.FC<WeatherProps> = ({ onBack, lang }) => {
  ...
  const fetchWeather = async (force: boolean = false) => {
    ...
    // Strategy: Fetch default weather immediately
  ...
  const performWeatherFetch = async (lat: number, lng: number, force: boolean) => {
    try {
      const data = await getLiveWeather(lat, lng, force, lang);
      setWeather(data);
      setHasError(false);
      ...
      // Don't set hasError true if we already have default weather
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const fetchDefaultWeather = async (force: boolean = false) => {
    try {
      const data = await getLiveWeather(23.8103, 90.4125, force, lang);
      setWeather(data);
      setHasError(false);
    } catch (e) {
      console.error("Weather default error:", e);
  ...
  const getFallbackIcon = () => {
    ...
    if (cond.includes('sun') || cond.includes('রোদ্র') || cond.includes('clear')) return "☀️";
    if (cond.includes('rain') || cond.includes('বৃষ্টি')) return "🌧️";
    if (cond.includes('cloud') || cond.includes('মেঘ')) return "⛅";
    if (cond.includes('fog') || cond.includes('কুয়াশা')) return "🌫️";
    return "🌤️";
  };

  const fetchGroundedReport = async () => {
    if (!weather) return;
    setIsReportLoading(true);
    try {
      const data = await generateGroundedWeatherReport(weather, lang);
      setReport(data);
    } catch (e) {
  ...
  useEffect(() => {
    if (activeTab === 'report' && !report && !isReportLoading) {
      fetchGroundedReport();
    }
  }, [activeTab]);
    ...
    <div className="max-w-4xl mx-auto bg-slate-50 min-h-screen pb-32 font-sans overflow-x-hidden">
      ...
      {!weather && isLoading ? (
        <div className="flex flex-col items-center justify-center py-40 animate-fade-in">
          ...
          <h2 className="text-2xl font-black text-slate-800 mt-8">{lang === 'bn' ? 'ডাটা সংগ্রহ হচ্ছে...' : 'Fetching Data...'}</h2>
          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] mt-2">BAMIS এবং Google Data Sync</p>
        </div>
      ) : hasError && !weather ? (
        <div className="flex flex-col items-center justify-center py-40 px-6 text-center animate-fade-in">
          <div className="text-6xl mb-6">📡</div>
          <h2 className="text-2xl font-black text-slate-800">{lang === 'bn' ? 'সংযোগ বিচ্ছিন্ন' : 'Sync Disconnected'}</h2>
          <p className="text-sm font-medium text-slate-500 mt-4 leading-relaxed max-w-xs">
            {lang === 'bn' ? 'আবহাওয়া সার্ভারের সাথে সংযোগ করা যাচ্ছে না। অনুগ্রহ করে আপনার ইন্টারনেট চেক করুন।' : 'Unable to connect to weather satellites. Please check your connection.'}
          </p>
          <button onClick={() => fetchWeather(true)} className="mt-8 bg-blue-600 text-white px-10 py-4 rounded-2xl font-black uppercase tracking-widest shadow-xl active:scale-95 transition-all">
            {lang === 'bn' ? 'পুনরায় চেষ্টা করুন' : 'Retry Connection'}
          </button>
        </div>
      ) : weather && (
        <div className="p-4 md:p-8 space-y-8 animate-fade-in">
          ...
          <div className="flex bg-white p-2 rounded-[2.5rem] shadow-xl border border-slate-100 overflow-x-auto scrollbar-hide">
            ...
            <button onClick={() => setActiveTab('report')} className={`flex-1 min-w-[120px] py-4 text-[10px] font-black uppercase rounded-[2rem] transition-all flex items-center justify-center space-x-2 ${activeTab === 'report' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-50'}`}>
              <span>📄</span><span>{lang === 'bn' ? 'সায়েন্টিফিক রিপোর্ট' : 'Agri Report'}</span>
            </button>
          </div>
          ...
          {activeTab === 'spraying' && (
            <div className="space-y-8 animate-fade-in">
              <div className={`bg-white rounded-[4rem] p-12 md:p-16 shadow-2xl border-t-[24px] border-${spray.color === 'emerald' ? 'emerald' : spray.color === 'amber' ? 'amber' : 'rose'}-600 text-center space-y-10 relative overflow-hidden group`}>
                <div className={`absolute top-0 right-0 w-48 h-48 bg-${spray.color === 'emerald' ? 'emerald' : spray.color === 'amber' ? 'amber' : 'rose'}-50 rounded-full -mr-24 -mt-24 opacity-60 group-hover:scale-110 transition-transform duration-1000`}></div>
                <div className={`w-40 h-40 rounded-[3rem] bg-${spray.color === 'emerald' ? 'emerald' : spray.color === 'amber' ? 'amber' : 'rose'}-50 flex items-center justify-center text-8xl shadow-inner mx-auto group-hover:scale-110 transition-transform`}>
                ...
                <div className="relative z-10">
                  ...
                  <h3 className={`text-5xl font-black leading-none mb-6 text-${spray.color === 'emerald' ? 'emerald' : spray.color === 'amber' ? 'amber' : 'rose'}-700`}>{spray.status}</h3>
          ...
          {/* Tab Content: Grounded Scientific Report */}
          {activeTab === 'report' && (
            <div className="space-y-6 animate-fade-in">
              {isReportLoading ? (
                <div className="bg-white rounded-[3rem] p-20 shadow-xl border border-slate-100 flex flex-col items-center justify-center text-center">
                  <div className="w-16 h-16 border-4 border-blue-100 border-t-blue-600 rounded-full animate-spin mb-6"></div>
                  <h3 className="text-xl font-black text-slate-800">{lang === 'bn' ? 'সায়েন্টিফিক রিপোর্ট তৈরি হচ্ছে...' : 'Generating Scientific Report...'}</h3>
                  <p className="text-sm font-bold text-slate-400 mt-2 uppercase tracking-widest">{lang === 'bn' ? 'BAMIS ও DAE ডাটা বিশ্লেষণ করা হচ্ছে' : 'Analyzing BAMIS & DAE Data'}</p>
                </div>
              ) : report ? (
                <div className="bg-white rounded-[3rem] p-8 md:p-12 shadow-xl border border-slate-100 relative overflow-hidden">
                  ...
                  <div className="flex justify-between items-start mb-10 relative z-10">
                    <div>
                      <div className="flex items-center space-x-2 mb-2">
                        ...
                        <h3 className="text-2xl font-black text-slate-800 tracking-tight">{lang === 'bn' ? 'অফিসিয়াল এগ্রি-মেট রিপোর্ট' : 'Official Agri-Met Report'}</h3>
                      </div>
                      <p className="text-[10px] font-black text-blue-500 uppercase tracking-[0.3em]">{weather.upazila}, {weather.district} • BAMIS Grounded</p>
                    </div>
                    <div className="flex space-x-2">
                      <button onClick={() => playTTS(report.text)} className={`p-4 rounded-2xl shadow-lg transition-all ${isPlaying ? 'bg-rose-500 text-white animate-pulse' : 'bg-slate-100 text-slate-400 hover:bg-blue-50'}`}>
                        {isPlaying ? '⏹️' : '🔊'}
                      </button>
                      <button onClick={fetchGroundedReport} className="p-4 bg-blue-50 text-blue-600 rounded-2xl shadow-lg active:scale-95 transition-all">
                        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
                      </button>
                    </div>
                  </div>
                  ...
                  {report.groundingChunks && report.groundingChunks.length > 0 && (
                    <div className="pt-8 border-t border-slate-100 relative z-10">
                      <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">{lang === 'bn' ? 'তথ্যসূত্র যাচাই (BAMIS Sources)' : 'Verification Sources'}</h4>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {report.groundingChunks.map((chunk, i) => chunk.web ? (
                          <a key={i} href={chunk.web.uri} target="_blank" rel="noopener noreferrer" className="flex items-center space-x-3 p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:border-blue-500 transition-all group">
                            <span className="w-8 h-8 bg-white rounded-lg flex items-center justify-center text-xs shadow-sm group-hover:bg-blue-600 group-hover:text-white transition-colors">🔗</span>
                            <span className="text-xs font-black text-slate-600 group-hover:text-blue-600 truncate">{chunk.web.title}</span>
                          </a>
                        ) : null)}
                      </div>
                    </div>
                ...
                <div className="bg-white rounded-[3rem] p-20 shadow-xl border border-slate-100 text-center">
                  <button onClick={fetchGroundedReport} className="bg-blue-600 text-white px-10 py-5 rounded-[2rem] font-black uppercase tracking-widest shadow-xl hover:bg-blue-700 transition-all">
                    {lang === 'bn' ? 'সায়েন্টিফিক রিপোর্ট তৈরি করুন' : 'Generate Scientific Report'}
                  </button>
                </div>
              )}
            </div>
