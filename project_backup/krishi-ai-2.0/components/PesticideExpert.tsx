...
const PesticideExpert: React.FC<any> = ({ onBack, onAction, onSaveReport, lang }) => {
  ...
  const [advice, setAdvice] = useState<{ text: string, groundingChunks: GroundingChunk[], officialSource?: string } | null>(null);
  ...
  const handleSearchSubmit = async (precision: boolean = false) => {
    ...
    try {
      ...
      // Fallback
      try {
        const fallback = await groundedAgriSearch("Pesticide product", query, lang);
        setAdvice({
          text: fallback.fullText || "Official search results returned.",
          groundingChunks: fallback.groundingChunks || [],
          officialSource: `(SAFE SEARCH) ${fallback.officialSource}`
        });
      } catch (f) { alert("Error fetching data."); }
  ...
  const handleSprayAdvisory = async () => {
    ...
    try {
      ...
      try {
        const fallback = await groundedAgriSearch(sprayCrop, `spray against ${sprayPest}`, lang);
        setAdvice({
          text: fallback.fullText || "Official search results returned.",
          groundingChunks: fallback.groundingChunks || [],
          officialSource: `(SAFE SEARCH) ${fallback.officialSource}`
        });
      } catch (f) { alert("Advisory generation failed."); }
    } finally { setIsLoading(false); }
  };
  ...
  const handleMixingAnalysis = async () => {
    ...
    try {
      ...
      try {
        const q = validItems.map(i => i.text).join(' and ');
        const fallback = await groundedAgriSearch("Pesticide mixing", `Safety of mixing ${q}`, lang);
        setAdvice({
          text: fallback.fullText || "Official search results returned.",
          groundingChunks: fallback.groundingChunks || [],
          officialSource: `(SAFE SEARCH) ${fallback.officialSource}`
        });
      } catch (f) { alert("Analysis failed."); }
    } finally { setIsLoading(false); }
  };
  ...
  const handleRotationSchedule = async () => {
    ...
    try {
      ...
      try {
        const fallback = await groundedAgriSearch(query, "pest resistance rotation schedule", lang);
        setAdvice({
          text: fallback.fullText || "Official search results returned.",
          groundingChunks: fallback.groundingChunks || [],
          officialSource: `(SAFE SEARCH) ${fallback.officialSource}`
        });
      } catch (f) { alert("Rotation schedule generation failed."); }
    } finally { setIsLoading(false); }
  };
