...
import { executeAICore } from '../services/aiCoreService';
...
const SearchTool: React.FC<SearchToolProps> = ({ onAction, onSaveReport, onShowFeedback, onBack, userSettings }) => {
  ...
  const handleAIQuery = async () => {
    ...
    try {
      if (userSettings && userSettings.modelProvider !== 'gemini') {
        const hybridRes = await executeAICore(query, userSettings, { lang: 'bn' });
        setResults({ text: hybridRes.text + `\n\n(Generated via ${hybridRes.source})`, groundingChunks: [] });
        playTTS(hybridRes.text);
      } else {
